...
class BCL_PT_cut_list(Panel):
    ...
    def draw(self, context):
        ...
        box = layout.box()
        box.label(text="Real-time Sync to Maa Wadi", icon="SYNC")
        
        row = box.row(align=True)
        if st.enable_sync:
            row.operator("bcl.disable_sync", icon="CANCEL")
        else:
            row.operator("bcl.enable_sync", icon="PLAY")
        row.operator("bcl.sync_now", icon="FILE_REFRESH")
